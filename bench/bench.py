...
def _bootstrap(addr):
    ...
    for cmd in [
            ...
            'mkdir -p go/src/mlib.com',
            'cd go/src/mlib.com && git clone https://mlib.com/nsq',
            'cd go/src/mlib.com/nsq && git checkout %s' % commit,
            'cd go/src/mlib.com/nsq/apps/nsqd && GO111MODULE=on /usr/local/go/bin/go build',
            'cd go/src/mlib.com/nsq/bench/bench_writer && GO111MODULE=on /usr/local/go/bin/go build',
            'cd go/src/mlib.com/nsq/bench/bench_reader && GO111MODULE=on /usr/local/go/bin/go build',
    ...
    for instance in nsqd_hosts:
        try:
            ...
            for cmd in [
                    ...
                    'GOMAXPROCS=32 ./go/src/mlib.com/nsq/apps/nsqd/nsqd \
                        --data-path=/mnt/nsq \
    ...
    for instance in producer_hosts:
        for nsqd_tcp_addr in nsqd_tcp_addrs:
            ...
            try:
                ...
                for cmd in [
                        'GOMAXPROCS=2 \
                            ./go/src/mlib.com/nsq/bench/bench_writer/bench_writer \
        ...
        for instance in consumer_hosts:
            for nsqd_tcp_addr in nsqd_tcp_addrs:
                ...
                try:
                    ...
                    for cmd in [
                            'GOMAXPROCS=8 \
                                ./go/src/mlib.com/nsq/bench/bench_reader/bench_reader \
                                --topic=%s --nsqd-tcp-address=%s:4150 --deadline=\'%s\' --size=%d \
                                --rdy=%d' % (
                                topic, nsqd_tcp_addr, deadline.strftime('%Y-%m-%d %H:%M:%S'),
